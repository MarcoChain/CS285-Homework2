...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            ...
            # In other words: value of (s_t, a_t) = sum_{t'=0}^T F^t' r_{t'}
    ...
    def _discounted_return(self, rewards):
        ...
        for t_prime in range(len(rewards)):
        	somma += self.gamma**t_prime * rewards[t_prime]
        
        list_of_discounted_returns = [somma] * len(rewards)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        # HINT2: it is possible to write a vectorized solution, but a solution
            # using a for loop is also fine avevo fatto la media dei return su tutti gli episodi
