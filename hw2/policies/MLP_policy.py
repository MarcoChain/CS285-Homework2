...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self,
        ...
        if self.discrete:
            self.logits_na = ptu.build_mlp(input_size=self.ob_dim,  #state s in input
                                           output_size=self.ac_dim, #action a from the policy as output
        ...
        if nn_baseline:
            self.baseline = ptu.build_mlp(
                input_size=self.ob_dim, #state s in input
                output_size=1,	 # V(s) as output
                n_layers=self.n_layers,
                size=self.size,
            )
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        return ptu.to_numpy(action_distribution.sample()) #scelgo l'azione per ogni stato basandomi sulla policy stocastica
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        if self.nn_baseline:
            ...
            baseline_predictions = self.baseline(observations).squeeze(1) # from [batch_size, 1]  ---> [batch_size]
